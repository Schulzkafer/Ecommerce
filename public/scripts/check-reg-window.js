   ...
   buttonCheckin.addEventListener('click', (event) => {
      ...
      xhr.onload = function () {
         ...
         if (xhr.status == 401) {
            ...
            let idU = res.id;
            setCookie('id', idU, { 'max-age': 2.592e+6 });

            let ema = res.email;
            setCookie('email', ema, { 'max-age': 2.592e+6 });

            let cre = res.credit;
            setCookie('credit', cre, { 'max-age': 2.592e+6 });

            let nam = res.name;
            setCookie('name', nam, { 'max-age': 2.592e+6 });

            let surnam = res.surname;
            setCookie('surname', surnam, { 'max-age': 2.592e+6 });
